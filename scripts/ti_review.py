...
def summarize_match(match_id, hero_cn):
...
def ensure_review(m, hero_cn, idx, force=False, publish=False):
    ...
    summary = summarize_match(m["match_id"], hero_cn)
    title = f"TI15 · {summary['radiant_team']} vs {summary['dire_team']} 复盘"
    review_md = call_llm(summary, title)
    # 用真实比赛时间（如有）
...
def main():
    ...
    ap.add_argument("--china", action="store_true", help="只生成含中国队的比赛")
    ap.add_argument("--all", action="store_true", help="批量生成所有 TI 比赛")
    ...
    if args.list:
        ...
        for m in sorted(matches, key=lambda x: x["match_id"], reverse=True):
            ...
            cn = "🇨🇳" if is_china_match(m) else "  "
            print(f"  {m['match_id']:<12}{done:<5}{cn} {m.get('radiant_name','?'):<20} vs {m.get('dire_name','?')}")
        return
    ...
    for m in sorted(targets, key=lambda x: x["match_id"], reverse=True):
        try:
            op, _ = ensure_review(m, hero_cn, idx, force=args.force, publish=args.publish)
            stats[op] += 1
        except Exception as e:
